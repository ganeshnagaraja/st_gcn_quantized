...
class IO():
    ...
    def load_weights(self):
        if self.arg.quantized_weights:
            self.model = self.io.load_weights(self.model, self.arg.quantized_weights,
                                              self.arg.ignore_weights)
        elif self.arg.weights:
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--quantized_weights', default=None, help='the quantized_weights will be used')
        # quantized_weights: ./checkpoints/quantized_model.pt
        parser.add_argument('--model_args', action=DictAction, default=dict(), help='the arguments of model')
        parser.add_argument('--weights', default=None, help='the weights for network initialization')
        parser.add_argument('--ignore_weights', type=str, default=[], nargs='+', help='the name of weights which will be ignored in the initialization')
        parser.add_argument('--mediapipe_pose_points', default=None)
        parser.add_argument('--openpose_17_keypoints', default=None)
